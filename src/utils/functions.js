...
export async function loadGuild(guildId, guildName, client) {
  if (!guildId) {
    logger.warn("devGuild is not set in config.json!");
  }
  try {
    const devGuild = await client.guilds.fetch(guildId);
    client[guildName] = devGuild;
    logger.log(
      `${guildName}: ${client[guildName].name} ${client[guildName].id}`,
    );
  } catch (e) {
...
export async function loadChannel(channelId, channelName, client) {
  if (!channelId) {
    return logger.warn(`${channelName} is not set in config.json!`);
  }
  try {
    const devChannel = await client.devGuild.channels.fetch(channelId);
    client[channelName] = devChannel;
    logger.log(
      `${channelName}: #${client[channelName].name} (${client[channelName].id})`,
    );
  } catch (e) {
  ...
  for (let i = 0; i < files.length; i++) {
    if (files[i].endsWith(".js")) {
      ...
      try {
        ...
        logger.error(
          `Failed to load the command ${commandName[commandName.length - 1]}: ${e}`,
        );
      }
    } else {
      commands.push(
        ...loadCommands(path.join(commandsPath, files[i]), accumulated),
      );
    }
  }
  return commands;
